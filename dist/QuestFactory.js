...
class QuestFactory {
    ...
    create(GameState, qid, player, state = []) {
        ...
        const instance = new Quest_1.Quest(GameState, quest.id, quest.config, player);
        instance.state = state;
        for (const goal of quest.config.goals) {
            const goalType = GameState.QuestGoalManager.get(goal.type);
            if (!goalType) {
                throw new Error(`QuestFactory did not find the goal with the type [${goal.type}]`);
            }
        ...
        instance.on('complete', () => {
            ...
            for (const reward of quest.config.rewards) {
                try {
                    ...
                    Logger_1.Logger.warn(`Error in quest ${qid} happened to ${player.name}.`);
