...
class Skill {
    ...
    constructor(id, config, state) {
        const { configureEffect = (_) => _, cooldown = null, effect = null, flags = [], info = (_) => { }, initiatesCombat = false, name, requiresTarget = true, resource = null /* format [{ attribute: 'someattribute', cost: 10}] */, run = (_) => { }, targetSelf = false, type = SkillType_1.SkillType.SKILL, options = {}, } = config;
    ...
    execute(args, player, target) {
        if (this.flags.includes(SkillFlag_1.SkillFlag.PASSIVE)) {
            throw new SkillErrors_1.PassiveError();
        }
        const cdEffect = this.onCooldown(player);
        if (this.cooldownLength && cdEffect) {
            throw new SkillErrors_1.CooldownError(cdEffect);
        }
        if (this.resource) {
            if (!this.hasEnoughResources(player)) {
                throw new SkillErrors_1.NotEnoughResourcesError();
            }
        }
    ...
    payResourceCost(player, resource) {
        ...
        const damage = new Damage_1.Damage(resource.attribute, resource.cost, player, this, {
            hidden: true,
        });
    ...
    createCooldownEffect() {
        ...
        const effect = this.state.EffectFactory.create('cooldown', {
        ...
        return {
            ...
            listeners: (state) => ({
                effectDeactivated: function () {
                    var _a;
                    if (!this.target) {
                        Logger_1.Logger.error('Cooldown effect has no target.');
                        return;
                    }
                    const skillName = typeof this.skill === 'string' ? this.skill : ((_a = this.skill) === null || _a === void 0 ? void 0 : _a.name) || 'unnamed skill';
                    Broadcast_1.Broadcast.sayAt(this.target, `You may now use <bold>${skillName}</bold> again.`);
                },
            }),
        };
    }
