...
import { EntityLoaderKeys } from './EntityLoaderRegistry';
...
export class BundleManager {
    ...
    addAttributes(attributes, errorPrefix) {
        for (const attribute of attributes) {
            ...
            if (attribute.formula) {
                formula = new AttributeFormula(attribute.formula.requires || [], attribute.formula.fn);
            }
    ...
    async loadArea(bundle, areaName, manifest) {
        ...
        definition.items = await this.loadEntities(bundle, areaName, EntityLoaderKeys.ITEMS, this.state.ItemFactory);
        Logger.verbose(`\t\tLOAD: NPCs...`);
        definition.npcs = await this.loadEntities(bundle, areaName, EntityLoaderKeys.NPCS, this.state.MobFactory);
        Logger.verbose(`\t\tLOAD: Rooms...`);
        definition.rooms = await this.loadEntities(bundle, areaName, EntityLoaderKeys.ROOMS, this.state.RoomFactory);
        Logger.verbose('\t\tDone.');
