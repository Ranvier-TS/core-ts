...
class EntityFactory {
  ...
  createByType(area, entityRef, Type) {
    let definition = this.getDefinition(entityRef);
    if (!definition) {
      // TODO: replace
      definition = this.getDefinition('starting-temple:debug');
   ...
   */
  modifyDefinition (entity, toSet, newProps) {
      ...
      for (const prop of Object.keys(newProps)) {
        ...
        if (toSet === 'behaviors') {
          if (Object.keys(newProps[prop])[0] === '...') {
            ...
            delete newProps[prop]['...'];
            entity[toSet].set(prop, Object.assign({}, {...entity[toSet].get(prop), ...newProps[prop]}));
            continue;
          }
          entity[toSet].set(prop, newProps[prop]);
          continue;
              ...
              if (Object.keys(newProps[prop])[0] === '...') {
                if (newProps[prop]['...'] === true) {
                  if (newProps[prop].value !== undefined) {
                    if (Array.isArray(newProps[prop].value)) {
                      entity[toSet][prop] = [...entity[toSet][prop], ...newProps[prop].value];
                  ...
                  delete newProps[prop]['...'];
                  if (newProps[prop].value !== undefined) {
                    this.modifyDefinition(entity[toSet], prop, newProps[prop].value);
                  } else {
                    this.modifyDefinition(entity[toSet], prop, newProps[prop]);
                  }
                  // console.log('deep array copied ~~>', prop)
                ...
                for (const innerProp of Object.keys(newProps[prop])) {
                  if (Object.keys(newProps[prop][innerProp])[0] === '...') {
                    if (newProps[prop][innerProp]['...'] === true) {
                      if (newProps[prop][innerProp].value !== undefined) {
                        if (Array.isArray(newProps[prop][innerProp].value)) {
                          entity[toSet][prop][innerProp] = [...entity[toSet][prop][innerProp], ...newProps[prop][innerProp].value];
                    ...
                    delete newProps[prop]['...'];
                    this.modifyDefinition(entity[toSet], innerProp, newProps[prop][innerProp]);
                  }
                }
              }
            }
